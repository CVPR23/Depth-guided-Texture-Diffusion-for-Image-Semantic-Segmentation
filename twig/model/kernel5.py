...
class ShapePropWeightRegressor(nn.Module):
    def __init__(self, in_channels, latent_dim):
        ...
        self.reg = nn.Conv2d(in_channels, self.latent_dim*25, kernel_size=1)
...
class MessagePassing(nn.Module):
    def __init__(self, latent_dim, img_size=384, k=5, max_step=6, sym_norm=False):
    ...
    def forward(self, input, weight):
        ...
        for i in range(max(h, w) if self.max_step < 0 else self.max_step):
            x = F.unfold(x, kernel_size=self.k, padding=2).view(n, c, self.size, h * w)
