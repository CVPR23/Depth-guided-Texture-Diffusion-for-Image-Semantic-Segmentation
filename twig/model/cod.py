...
class cod(BaseModel):
    ...
    def forward(self, raw, input, label, depth, mode='loss'):  
        ...
        if mode == 'loss':             
            ...
            loss = loss_p1 + loss_P2 + loss_3 * 1#0.02
            ...
            def save_tensor_as_image(tensor, name):
                ...
                return file_path
            def save_tensor_as_1cimage(tensor, name):
                file_path = os.path.join(output_dir, f'{name}.png')

                # 调整tensor的尺寸，确保它是[B, C, H, W]格式
                if tensor.dim() == 2:  # 如果是单通道图像，没有批次和通道维度
                    tensor = tensor.unsqueeze(0).unsqueeze(0)  # 添加批次和通道维度
                elif tensor.dim() == 3:  # 如果只有批次维度
            ...
            def denormalize(tensor, mean, std):
                ...
                return tensor        
            def enhance_contrast(image_tensor, contrast_factor=2):
                # 计算均值
                mean = torch.mean(image_tensor)

                # 提升对比度
                enhanced_tensor = (image_tensor - mean) * contrast_factor + mean
            ...
            temp = F.interpolate(embedding1, scale_factor=1, mode='bilinear')
            temp = enhance_contrast(temp)
            diffusion_path = save_tensor_as_1cimage(temp, f'{name}_diffusion')
...
class our_init(Hook):
    ...
    def before_val(self, runner):
        ...
        # Load checkpoint of hitnet 
        pretrain = 'output/visual_sod/epoch_50.pth'
        checkpoint = torch.load(pretrain, map_location='cpu')
        print("Load pre-trained checkpoint from: %s" % pretrain)
        if 'model' in checkpoint:
            checkpoint = checkpoint['model']
...
class MessagePassing(nn.Module):
    def __init__(self, latent_dim, img_size=384, k=7, max_step=4, sym_norm=False):
...
class prompt_encoder(nn.Module):
    ...
    def forward(self, image, cues, cross=False):
        ...
        x = self.fft(image, self.freq_nums)
        ...
        # propagation  
        xx = F.interpolate(x, size=[H,H])
        weights = self.propagation_weight_regressor(xx)
