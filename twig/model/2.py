...
class yy(Hook):
    ...
    #     # Load checkpoint of hitnet 
    #     pretrain = 'output/cod_prompt_embeddim/epoch_80.pth'
    #     checkpoint = torch.load(pretrain, map_location='cpu')
    #     print("Load pre-trained checkpoint from: %s" % pretrain)
    #     if 'model' in checkpoint:
    #         checkpoint = checkpoint['model']
...
class Depth_prompt(nn.Module):
    def __init__(self, scale_factor, input_dim, embed_dim, depth, win_size, fusion=False):
        ...
        self.shared_mlp = nn.Linear(self.embed_dim//self.scale_factor,  self.embed_dim)#self.input_dim//self.scale_factor, self.embed_dim)
    ...
    def forward(self, cues, cross=False):
        N, C, H, W = cues.shape
        ori_cues= cues
        prompts = []      
        # prompt generating
...
class PyramidVisionTransformerImpr(nn.Module):
    ...
    def forward_features(self, x, pred_normal):
        ...
        weights = self.propagation_weight_regressor_0(x.permute(0,2,1).reshape(B,C,H,W))

        for i, blk in enumerate(self.block1):
            embedding = self.encoder_0(F.interpolate(depth[i].permute(0,2,1).reshape((B,64,self.cross_size,self.cross_size)), size=(H,W), mode='bilinear'))
            embedding = self.message_passing_0(embedding, weights)
            depth[i] = self.decoder_0(embedding) 
            depth[i] = depth[i].flatten(2).permute(0,2,1)
        ...
        weights = self.propagation_weight_regressor_1(x.permute(0,2,1).reshape(B,C,H,W))

        for i, blk in enumerate(self.block2):
            embedding = self.encoder_1(F.interpolate(depth[i].permute(0,2,1).reshape((B,128,self.cross_size,self.cross_size)), size=(H,W), mode='bilinear'))
        ...
        weights = self.propagation_weight_regressor_2(x.permute(0,2,1).reshape(B,C,H,W))

        for i, blk in enumerate(self.block3):
            embedding = self.encoder_2(F.interpolate(depth[i].permute(0,2,1).reshape((B,320,self.cross_size,self.cross_size)), size=(H,W), mode='bilinear'))
        ...
        weights = self.propagation_weight_regressor_3(x.permute(0,2,1).reshape(B,C,H,W))

        for i, blk in enumerate(self.block4):
            embedding = self.encoder_3(F.interpolate(depth[i].permute(0,2,1).reshape((B,512,self.cross_size,self.cross_size)), size=(H,W), mode='bilinear'))
